  ...
  return (
    <div className="stagger-children">
      ...
      <div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        />
        <SummaryCard label="检测次数" value={String(items.length)} />
        <SummaryCard label="参考区间" value={refRange} />
      ...
      <div className="card p-4">
        ...
        <p className="text-[13px] font-medium text-neutral-800 font-body">
          {trendDirection === 'stable'
            ? `${metricName}保持稳定`
            : `${metricName}${trendDirection === 'rising' ? '上升' : '下降'}（${changePct > 0 ? '+' : ''}${Math.round(changePct)}%）`}
        </p>
        <p className="text-[11px] text-neutral-500 font-body mt-1">
          基于该期间 {observations.length} 次检测结果。
