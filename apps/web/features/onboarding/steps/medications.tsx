...
export function MedicationsStep({ data, onChange, onNext, onBack, onSkip, direction }: MedicationsStepProps) {
  ...
  return (
    <StepLayout
      ...
      direction={direction}
      title="用药与过敏"
      subtitle="列出你目前正在使用的药物以及已知过敏。"
      why="为什么？帮助我们提示潜在相互作用，保障用药安全。"
      wide
      footer={
      ...
      <div className="space-y-6">
              ...
              {data.medications.map((med, i) => (
                <div key={i} className="flex items-start gap-2 border border-neutral-200 bg-white p-3">
                  <div className="grid flex-1 grid-cols-3 gap-2">
                    <input type="text" value={med.name} onChange={(e) => updateMed(i, 'name', e.target.value)} className={inputClass} placeholder="药物名称" />
                    ...
                    <input type="text" value={med.frequency} onChange={(e) => updateMed(i, 'frequency', e.target.value)} className={inputClass} placeholder="频次" />
                  </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {data.noAllergies ? '✓ No known allergies (NKDA)' : '我没有已知过敏'}
            </button>
          </div>
        </div>
      </div>
