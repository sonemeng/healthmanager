...
export function AnimatedEmptyState({
  ...
  cardContent?:
    | ReactNode
    | (({ index, icon }: { index: number; icon: IconType }) => ReactNode);
  ...
  return (
    <div
      className={cn(
        "flex flex-col items-center justify-center gap-6 card px-4 py-10 md:min-h-[420px]",
        className,
      )}
    >
      ...
      <div className="max-w-xs text-pretty text-center">
        <span className="text-lg font-medium text-neutral-900 font-display">
          {title}
        </span>
        {!!description && (
          <p className="mt-2 text-pretty text-sm text-neutral-500 font-display">
            {description}
          </p>
        )}
      </div>
      <div className="flex items-center gap-2">
        ...
        {learnMoreHref && (
          <Link
            href={learnMoreHref}
            className={cn(
              "card px-4 py-2 text-sm font-medium text-neutral-700 hover:bg-neutral-50 transition-colors",
              learnMoreClassName,
            )}
          >
            Learn more
...
function Card({ children }: PropsWithChildren) {
  return (
    <div className="mt-4 flex items-center gap-3 card p-4">{children}</div>
  );
}
