...
export function LabsPreviewContent({ items }: LabsPreviewContentProps) {
  if (items.length === 0) {
    return <p className="text-[13px] text-neutral-400 font-body">暂无化验结果</p>;
  }
  ...
  return (
    <div>
      <div className="flex items-baseline gap-3">
        ...
        <span className="text-[13px] text-neutral-500 font-body">项指标</span>
        {abnormalCount > 0 && (
...
export function MedicationsPreviewContent({ items }: MedicationsPreviewContentProps) {
  if (items.length === 0) {
    return <p className="text-[13px] text-neutral-400 font-body">尚未添加药物</p>;
  }
  ...
  return (
    <div>
      <div className="flex items-baseline gap-3">
        ...
        <span className="text-[13px] text-neutral-500 font-body">在用</span>
      </div>
...
export function TimelinePreviewContent({ eventCount, latestEvent }: TimelinePreviewContentProps) {
  if (eventCount === 0) {
    return <p className="text-[13px] text-neutral-400 font-body">暂无事件</p>;
  }
  ...
  return (
    <div>
      <div className="flex items-baseline gap-3">
        ...
        <span className="text-[13px] text-neutral-500 font-body">条事件</span>
      </div>
      {latestEvent && (
...
export function UploadsPreviewContent({ items }: UploadsPreviewContentProps) {
  if (items.length === 0) {
    return <p className="text-[13px] text-neutral-400 font-body">暂无上传记录</p>;
  }

  const latest = items[0]!;
  const statusMap: Record<string, { status: 'normal' | 'warning' | 'info' | 'neutral'; label: string }> = {
    completed: { status: 'normal', label: '已完成' },
    processing: { status: 'info', label: '处理中' },
    failed: { status: 'warning', label: '失败' },
  };
  ...
  return (
    <div>
      <div className="flex items-baseline gap-3">
        ...
        <span className="text-[13px] text-neutral-500 font-body">次上传</span>
      </div>
      <div className="mt-3 flex items-center justify-between gap-2">
